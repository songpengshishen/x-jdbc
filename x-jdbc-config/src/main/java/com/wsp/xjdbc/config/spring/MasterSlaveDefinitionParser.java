...
import com.wsp.xjdbc.config.bean.MasterDataSourceConfigBean;
import com.wsp.xjdbc.config.bean.MasterSlaveDataSourceConfigBean;
import com.wsp.xjdbc.config.bean.SlaveDataSourceConfigBean;
...
public class MasterSlaveDefinitionParser extends AbstractSimpleBeanDefinitionParser {
    ...
    protected void doParse(Element element, ParserContext parserContext, BeanDefinitionBuilder builder) {
        ...
        //当前容器是否已有重复BeanID
        checkBeanDuplicate(parserContext, idVal);
        //添加master数据源配置
        ...
        //添加strategy配置
        builder.addPropertyValue("strategyConfig", createStrategy(element, parserContext));
        builder.addPropertyValue("dataSourceFactoryRef", element.getAttribute("dataSourceFactoryRef"));
    ...
    private BeanDefinition createMasterDataSourceDefinition(Element element, ParserContext parserContext) {
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(MasterDataSourceConfigBean.class);
        ...
        builder.addPropertyValue("targetDataSourceRef", masterElement.getAttribute("targetDataSourceRef"));
    ...
    private BeanDefinition createSlaveDataSource(Element element, ParserContext parserContext) {
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(SlaveDataSourceConfigBean.class);
        ...
        builder.addPropertyValue("targetDataSourceRef", element.getAttribute("targetDataSourceRef"));
        beanPostProcess(builder);
        return builder.getBeanDefinition();
    }
    ...
    private BeanDefinition createStrategy(Element element, ParserContext parserContext) {
        Element strategyElement = getStrategyElement(element);
        if (null != strategyElement) {
            BeanDefinitionBuilder builder = getBeanDefinitionBuilder(MasterSlaveStrategyConfig.class);
            builder.addPropertyValue("route",strategyElement.getAttribute("route" ));
            beanPostProcess(builder);
            return builder.getBeanDefinition();
        }
        return null;
     ...
     * @return
     */
    private Element getStrategyElement(Element element) {
        //因为xsd的校验,程序走到这里肯定是有标签的,并且slaveDataSources只有一个.
        List<Element> elements = loopFindElement(element, "strategy");
        Element e = null;
    ...
    private void checkBeanDuplicate(ParserContext parserContext, String idVal) {
